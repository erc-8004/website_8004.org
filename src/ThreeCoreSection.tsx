          ...
          <div className="grid md:grid-cols-3 gap-8 mb-12 relative">
            ...
            {primitives.map((primitive, index) => {
              ...
              return (
                <div
                  key={primitive.id}
                  className={`bg-white p-10 rounded-2xl border border-[#e4e4e7] card-shadow ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                  style={{ transitionDelay: `${200 + index * 100}ms` }}
                >

                  <div className="flex justify-center mb-6">
                    <div className="w-16 h-16 rounded-2xl bg-[#f3f0ff] flex items-center justify-center">
                      <Icon className="w-8 h-8 text-[#4C2A85]" />
                    </div>
                  </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {primitives.map((primitive, index) => {
              ...
              return (
                <div
                  key={primitive.id}
                  className={`bg-white p-8 rounded-2xl border-l-4 border-[#e4e4e7] card-shadow ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                  style={{ transitionDelay: `${700 + index * 100}ms` }}
                >
                  <div className="flex items-start gap-3 mb-4">
                    <DetailIcon className="w-5 h-5 mt-1 flex-shrink-0 text-[#a78bfa]" />
